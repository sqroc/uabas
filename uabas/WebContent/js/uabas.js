		...
		.dataTable(
				{
					...
					"bSort" : false, // 是否使用排序
					"sAjaxSource" : "admin!getAllAdmins.action?rand="
							+ Math.random(),
					"aoColumns" : [
							{
								"mDataProp" : "aid"
							...
							},
							{
								...
								"fnRender" : function(obj) {
									...
									var sReturn = '<a class="btn btn-info useredit" href="#" onclick="useredit(this)" id="'
											+ aid
											+ '"><i class="icon-edit icon-white"></i>编辑</a>'
											+ '<a class="btn btn-danger userdelete" href="#" onclick="userdelete(this)" id="'
											+ aid
											+ '"><i class="icon-trash icon-white"></i>删除</a>';
									return sReturn;
								}
							// "sDefaultContent": '<a href=""
...
function websitecode(e) {
	...
	/*
	 * $.post("website!loadByAid.action", { "id" : $(e).attr('id') },
	 * function(data) { $("#name2").val(data.aaData[0].name);
	 * $("#aid").val(data.aaData[0].id); $("#url2").val(data.aaData[0].url);
	 * $("#description2").val(data.aaData[0].description); }, "json");
	 */
		...
		.dataTable(
				{
					...
					"bFilter" : false, // 是否使用搜索
					"bSort" : false, // 是否使用排序
					"sAjaxSource" : "website!getAllWebsites.action?rand="
							+ Math.random(),
					"aoColumns" : [
							{
								"mDataProp" : "id"
							...
							},
							{
								...
								"fnRender" : function(obj) {
									...
									var sReturn = '<a class="btn btn-success useredit" href="#" onclick="websitecode(this)" id="'
											+ aid
											+ '"><i class="icon-zoom-in icon-white"></i>获取代码</a>'
											+ '<a class="btn btn-info useredit" href="#" onclick="websiteedit(this)" id="'
											+ aid
											+ '"><i class="icon-edit icon-white"></i>编辑</a>'
											+ '<a class="btn btn-danger userdelete" href="#" onclick="websitedelete(this)" id="'
											+ aid
											+ '"><i class="icon-trash icon-white"></i>删除</a>';
									return sReturn;
								}
...
if ($("#webvisits").length) {
	...
	pages.push([ 15, 111 ]);
	/*
	 * for (var i = 0; i < 14; i += 0.5) { sin.push([i, Math.sin(i)/i]);
			...
			function(event, pos, item) {
				...
				if (item) {
					if (previousPoint != item.dataIndex) {
						...
						showTooltip(item.pageX, item.pageY, "本月" + x + "号的"
	...
	$("#webvisits").bind(
			...
			function(event, pos, item) {
				if (item) {
					$("#clickdata").text(
							"本月" + item.datapoint[0] + "号的" + item.series.label
									+ "为" + item.datapoint[1]);
...
// 浏览器统计
$.post("statistics!getbrowser.action", {
	...
	if (datars.success == true) {
		var data = [ {
			...
			data : datars.browsertype.ie
		}, {
			label : "其他",
			data : datars.browsertype.other
		}, {
			label : "Safari",
			data : datars.browsertype.safari
		}, {
			label : "Firefox",
			data : datars.browsertype.firefox
		}, {
			label : "Chrome",
			data : datars.browsertype.chrome
		} ];
...
$.post("statistics!getos.action", {
	...
	if (datars.success == true) {
		var data2 = [ {
			...
			label : "Linux",
			data : datars.ostype.linux
		}, {
			label : "MAC",
			data : datars.ostype.mac
		}, {
			label : "其他",
			data : datars.ostype.other
		} ];
					...
					"aoColumns" : [
							...
							},
							{
								...
								"fnRender" : function(obj) {
									...
									var sReturn = '<a class="btn btn-success useredit" href="#" onclick="showmousedraw(this)" id="'
											+ rid
											+ '"><i class="icon-zoom-in icon-white"></i>查看路径图</a>'
											+ '<a class="btn btn-danger userdelete" href="#" onclick="recorddelete(this)" id="'
											+ rid
											+ '"><i class="icon-trash icon-white"></i>删除记录</a>';
									return sReturn;
								}
							// "sDefaultContent": '<a href=""
							// class="editor_edit">Edit</a> / <a href=""
